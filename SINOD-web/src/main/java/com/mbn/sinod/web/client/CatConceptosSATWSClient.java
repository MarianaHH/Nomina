 ...
 * @author ambrosio
 */
public class CatConceptosSATWSClient {
    private static final RestTemplate TEMPLATE = new RestTemplate();
    private static final ResourceBundle BUNDLE = ResourceBundle.getBundle(ResourceBundles.CONFIG.getBundleName());
    private static final Logger logger = Logger.getLogger(CatConceptosSATWSClient.class.getName());

    public static List<Tsgnomconceptosat> listarConceptosSAT() {
        JSONObject json;
        try {
            ...
            URI uri = URIBuilder.buildWSUri(BUNDLE.getString(StaticsConstants.WS_LISTA_CATCONCEPTOSSAT), paramMap);
            CatConceptosSATDTO dto = TEMPLATE.getForObject(uri, CatConceptosSATDTO.class);
            return dto.getListarConceptosSAT();
